...
import { bugServiceData } from '../utils/Data';

interface BugsServicesProps {
  onServiceClick: (service: any) => void;
}

const BugsServices: React.FC<BugsServicesProps> = ({ onServiceClick }) => {
  return (
    <section id="böcek-hizmetleri" className="py-16 bg-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
            Böcek İlaçlama Hizmetlerimiz
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Ev, iş yeri ve depolarda etkili böcek kontrol çözümlerimiz
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {bugServiceData.map((service) => (
            <div
              key={service.id}
